...
while (keepsearching) {
    ...
    if (answer.product_property === "Title" && answer.productTitle) {
        const searchTitle = products.filter(products => products.title.toLowerCase().includes(answer.productTitle.toLowerCase()));
    ...
    if (answer.product_property === "Description" && answer.productDescription) {
        const searchDescription = products.filter(products => products.description.toLowerCase().includes(answer.productDescription.toLowerCase()));
    ...
    if (answer.product_property === "Price" && answer.productPrice) {
        const searchPrice = products.filter(products => products.price === Number(answer.productPrice));
    ...
    if (answer.product_property === "Rating" && answer.productRating) {
        const searchRating = products.filter(products => products.rating >= 4);
